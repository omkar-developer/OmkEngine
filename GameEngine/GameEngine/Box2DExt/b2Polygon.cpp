...
static const float32 toiSlop = 0;//0.0f;
...
//	printf("Adding...\n");
    ...
    for (int32 i = 0; i < nVertices; ++i) {
		...
		//((b2PolygonShape*)pd.shape)->m_vertices[ind] = vecs[i];
//		printf("%f, %f\n",vecs[i].x,vecs[i].y);
    }
//	print();
	//((b2PolygonShape*)pd.shape)->m_vertexCount = nVertices-offset;
	((b2PolygonShape*)pd.shape)->Set(vecs, nVertices);
    delete[] vecs;
}
...
b2FixtureDef* DecomposeConvexAndAddTo(b2Polygon* p, b2Body* bd,
...
//		printf("npolys: %d",nPolys);
        ...
        for (int32 i = 0; i < nPolys; ++i) {
            b2FixtureDef* toAdd = &pdarray[i+extra];
			 ...
			 toAdd->shape = new b2PolygonShape;
...
/*b2Polygon TraceEdge(b2Polygon* p, bool debug) {
	...
	const float32 EPS_TOL = .001f;//FLT_EPSILON;//.0001f;
...
b2Polygon TraceEdge(b2Polygon* p){
	...
	const float32 COLLAPSE_DIST = FLT_EPSILON;
	for (int32 i=0; i < nNodes; ++i){
...
b2PolyNode* b2PolyNode::GetRightestConnection(b2PolyNode& incoming){
	...
	for (int i=0; i<nConnected; ++i){
		...
		b2Assert (testLength > FLT_EPSILON);
