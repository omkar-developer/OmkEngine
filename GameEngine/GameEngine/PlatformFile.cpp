...
//
//	if(flags&Create_CreateAlways)
//	{
//		FILE* h = fopen(tmp, "wb+");
//		fwrite((void*)"okk", 1, 3, h);
//		fflush(h);
//		fclose(h);
...
//
//#ifdef ANDROID
//	__android_log_print(ANDROID_LOG_VERBOSE, "native", "Opening file>>>>>>>>>>>>>>>>>>>>>>>%s", tmp);
//#endif
//
...
bool C_File::IsGood()
{
	...
	if(ferror(m_handle)) return false;
	return true;
}
...
bool C_File::Open(const wchar_t* filename, Access access, CreationFlags flags, ShareMode share)
{
	...
	if(access&Access_Read) accesst = "rb";
	if(access&Access_Write) accesst = "wb";
	if(access&Access_ALL) accesst = "rb+";
	...
	m_handle = fopen(tmp, accesst.c_str());
...
bool C_File::Seek(long long val, SeekMode mode, long long* newdest)
{
	...
	{
		int mmode;
		if(mode==Seek_Set) mmode = SEEK_SET;
		else if(mode==Seek_Current) mmode = SEEK_CUR;
		else if(mode==Seek_End) mmode = SEEK_END;

		if(fseek(m_handle, val, mmode))
		{
			return false;
		}
		else
